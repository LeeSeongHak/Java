...
var cnt = 0;
var wayPointCnt = 0;
var arrWayPoint = new Array(wayPointCnt);
var markerWaypoint = new Array(wayPointCnt);
...
function initTmap() {
	...
	markerChoice = $('input[name="markerChoice"]:checked').val();
    
	if(markerChoice == 1){
        markerLayer = new Tmap.Layer.Markers();//마커 레이어 생성
        map.addLayer(markerLayer);//map에 맵 레이어를 추가합니다.
    	map.events.register("click", map, onClickStart);//map 클릭 이벤트를 등록합니다.
    }
    if(markerChoice == 2){
    	markerLayer = new Tmap.Layer.Markers("waypoint");//마커 레이어 생성
    	map.addLayer(markerLayer);//map에 마커 레이어 추가
    	map.events.register("click", map, onClickWaypoint);//경유지가 있을 경우 실행합니다.
    }
    if(markerChoice == 3){
        markerLayer = new Tmap.Layer.Markers();//마커 레이어 생성
        map.addLayer(markerLayer);//map에 맵 레이어를 추가합니다.
    	map.events.register("click", map, onClickEnd);
    }
...
//경유지가 있을 때 발생하는 이벤트 함수입니다.
function onClickWaypoint(e){
	lonlat = map.getLonLatFromViewPortPx(e.xy).transform("EPSG:3857", "EPSG:4326");//클릭 부분의 ViewPortPx를 LonLat 좌표로 변환합니다.
	
	arrWayPoint[wayPointCnt] = map.getLonLatFromViewPortPx(e.xy).transform("EPSG:3857", "EPSG:4326");
	
	
	var size = new Tmap.Size(24, 38);//아이콘 사이즈 설정
	var offset = new Tmap.Pixel(-(size.w/2), -(size.h));//아이콘 중심점 설정
	var icon = new Tmap.Icon('http://tmapapis.sktelecom.com/upload/tmap/marker/pin_b_m_p.png',size, offset);//마커 아이콘 설정
	markerWaypoint[wayPointCnt] = new Tmap.Marker(lonlat.transform("EPSG:4326", "EPSG:3857"), icon);//마커 생성
	markerLayer.addMarker(markerWaypoint[wayPointCnt]);//마커 레이어에 마커 추가
	
	wayPointCnt = wayPointCnt + 1;
}

//도착지를 선택한 후 클릭했을 때 발생하는 이벤트 함수입니다.
function onClickEnd(e){
	...
	lola(lonlat1, arrWayPoint, lonlat2);
}


function lola(lonlat1, arrWayPoint, lonlat2){
	...
	var w_lonLat = new Array(wayPointCnt);
	for(var i = 0; i <= wayPointCnt; i++){
		w_lonLat[i] = new Tmap.LonLat(arrWayPoint[i].lon, arrWayPoint[i].lat); //경유지 좌표입니다.
	}
	...
	var a = tData.getRoutePlan(s_lonLat, w_lonlat, e_lonLat, optionObj);//경로 탐색 데이터를 콜백 함수를 통해 XML로 리턴합니다.
...
<body onload="initTmap()">
	...
	<form action="markerCheck" id="markerCheck">
	버튼 선택<br>
		<input type="radio" name='markerChoice' value='1' checked="checked">출발지
		<input type="radio" name='markerChoice' value='2'>경유지
		<input type="radio" name='markerChoice' value='3'>도착지
		<input type="button" id="bt1" value="변경" onclick="initTmap()">
	</form>
